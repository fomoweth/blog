...
import { cn, parseYears } from "@/lib/utils";
...
export default function About({ items }: Props) {
	...
	const renderItem = useCallback(
		(target: string) => {
			...
			return (
				<Link
					className="link z-10 text-[#0847F7] decoration-[0.1em]"
					href={skill.link}
	...
	return (
		<div
			className={cn(
				"mx-auto flex h-full w-full max-w-screen-2xl flex-col space-y-4 p-8 md:p-16",
			)}
		>
			...
			<motion.div
				className={cn(
					"mx-auto flex h-full flex-col gap-4 text-pretty text-base tracking-tight text-gray-600 md:gap-2 md:px-8 md:text-lg md:tracking-normal lg:gap-4 lg:text-xl lg:tracking-wide xl:text-2xl [&>p]:leading-relaxed",
				)}
			>
				<motion.p
				...
				>
					...
					programming languages such as {renderItem("solidity")} and{" "}
					{renderItem("typescript")}, and skilled in smart contract
					development tools such as {renderItem("foundry")} and{" "}
					{renderItem("hardhat")}.
				</motion.p>
